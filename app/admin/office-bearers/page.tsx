...
import {
  ...
  OfficeBearerRole,
  OBPosition,
} from "@/app/context/MockDataContext";
import { createClient } from "@/lib/supabase/client";

// Default + custom positions
const DEFAULT_POSITIONS: OBPosition[] = [
  "President",
  "General Secretary",
  "Press Secretary",
  "Treasurer",
...
export default function AdminOfficeBearersPage() {
  const {
    ...
    users,
    addOfficeBearerRole,
    updateOfficeBearerRole,
    removeOfficeBearerRole,
  } = useMockData();
    ...
    const getManagedSociety = async () => {
      ...
      if (user) {
        ...
        if (profile) {
          ...
          setUserData({ id: user.id, role: managed?.role });
        }
      }
      setIsLoading(false);
    };
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-white dark:bg-zinc-950">
        ...
        <p className="text-zinc-500 dark:text-zinc-400 max-w-sm">
          Only Society Admins can manage Office Bearers.
        </p>
      </div>
    );
  }
    ...
    <div className="h-full flex flex-col pt-4 px-4 md:px-8 pb-8 overflow-y-auto custom-scrollbar">
      <header className="mb-8 md:mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 dark:text-zinc-400 text-sm md:text-base">
            Manage society-level roles. Presidents access all teams; other OBs
            access only their assigned teams.
          </p>
        </div>
            ...
            <div className="space-y-4 mt-2">
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full bg-background dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-800 rounded-lg p-2.5 text-sm text-zinc-900 dark:text-zinc-100 outline-none focus:ring-1 focus:ring-rose-500"
                >
              ...
              {!isPresident && (
                <div className="space-y-2">
                  ...
                  <p className="text-[11px] text-zinc-400">
                    This OB will only access the selected teams.
                  </p>
                  <div className="grid grid-cols-2 gap-2 max-h-36 overflow-y-auto">
                    {teams.map((t) => (
                      <button
                        key={t.id}
                        onClick={() =>
                          toggleTeam(t.id, selTeamIds, setSelTeamIds)
                        }
                      ...
                      >
                        <span
                          className={`h-2 w-2 rounded-full ${t.color} shrink-0`}
                        />
                        {t.name}
                      </button>
                    ))}
              ...
              {isPresident && (
                <div className="bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-lg p-3 text-xs text-amber-700 dark:text-amber-400">
                  👑 President role grants access to <strong>all teams</strong>{" "}
                  automatically.
                </div>
              )}
                ...
                <div className="grid grid-cols-2 gap-2 max-h-36 overflow-y-auto">
                  {teams.map((t) => (
                    <button
                      key={t.id}
                      onClick={() =>
                        toggleTeam(t.id, editTeamIds, setEditTeamIds)
                      }
        ...
        <div className="flex flex-wrap gap-3">
          {[
            ...
            return (
              <div
                key={r.pos}
                className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs ${s.bg} ${s.text} ${s.border}`}
              >
                <span>{s.icon}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {officeBearers.map((ob) => {
          ...
          const assignedTeams = isPresident
            ? teams
            : teams.filter((t) => ob.assignedTeamIds.includes(t.id));
          return (
            <div
              key={ob.id}
              className="bg-white dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800/60 rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition"
            >
              <div
                className={`h-1.5 w-full ${isPresident ? "bg-gradient-to-r from-amber-400 to-amber-500" : "bg-gradient-to-r from-zinc-300 to-zinc-400 dark:from-zinc-600 dark:to-zinc-700"}`}
              />
              <div className="p-5">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <Avatar
                      className={`h-12 w-12 border-2 ${isPresident ? "border-amber-200 dark:border-amber-500/30" : "border-zinc-200 dark:border-zinc-700"} shadow-sm`}
                    >
                  ...
                  <DropdownMenu>
                    ...
                    >
                      ...
                      <DropdownMenuItem
                        onClick={() => removeOfficeBearerRole(ob.id)}
                ...
                {/* Position badge */}
                <div
                  className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold border mb-4 ${s.bg} ${s.text} ${s.border}`}
                >
                  {s.icon} {ob.position}
                </div>
                ...
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-zinc-400 mb-2 font-semibold">
                    {isPresident
                      ? "Full Access"
                      : `Team Access (${assignedTeams.length})`}
                  </p>
                  <div className="flex flex-wrap gap-1.5">
                    {isPresident ? (
                      <span className="text-xs px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20 font-medium">
                        All Teams
                      </span>
                      ...
                      assignedTeams.map((t) => (
                        <span
                          key={t.id}
                          className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700"
                        >
                          <span
                            className={`h-1.5 w-1.5 rounded-full ${t.color}`}
                          />
                          {t.name}
                        </span>
                      ))
