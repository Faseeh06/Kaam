...
export default function AdminUsersPage() {
  ...
  const activeUsers = users.filter((u) => {
    if (
      ...
      return false;
    if (isScopedOB) {
      // Filter down to specific teams
      const allowedTeamIds = teams
        .filter(
          (t) =>
            (t as any).society_id === managedSocietyId &&
            myOBRecord.assignedTeamIds.includes(t.id),
        )
        .map((t) => t.id);
  ...
  return (
    <div className="h-full flex flex-col pt-4 px-4 md:px-8 pb-8 overflow-y-auto custom-scrollbar">
        ...
        <div className="flex items-center gap-3">
          <div className="relative group">
            ...
            <input
              ...
              className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg pl-9 pr-4 py-2 text-sm text-zinc-900 dark:text-white outline-none focus:border-rose-500/50 focus:ring-1 focus:ring-rose-500 transition w-full md:w-64"
            />
          </div>
          <Button
            variant="outline"
              ...
              <DialogContent className="sm:max-w-md border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950">
                <DialogHeader>
                  ...
                  <DialogDescription className="text-zinc-500 dark:text-zinc-400">
                    Share this unique sign-up link. Once they register, they
                    will appear in your pending approvals.
                  </DialogDescription>
                </DialogHeader>
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <Button
                        ...
                        className="px-3 border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 text-zinc-700 dark:text-zinc-300"
                      >
                        {codeCopied ? (
                          <Check className="h-4 w-4 text-emerald-500" />
                        ) : (
        ...
        {!isScopedOB && filteredPending.length > 0 && (
          <div>
            <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100 mb-4 flex items-center gap-2">
              ...
              <Badge
                variant="secondary"
                className="bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-500"
              >
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody>
                    {filteredPending.map((user) => (
                      <tr
                        key={user.id}
                        className="border-b border-zinc-100 dark:border-zinc-800/50 hover:bg-zinc-50/80 dark:hover:bg-zinc-900/30 transition"
                      >
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <Avatar className="h-9 w-9 border border-zinc-200 dark:border-zinc-800">
                              <AvatarFallback className="bg-rose-100 dark:bg-rose-500/20 text-rose-600 dark:text-rose-500 text-xs font-medium">
                                {user.name.charAt(0)}
                              </AvatarFallback>
                            </Avatar>
                            <div>
                  ...
                  {activeUsers.map((user) => (
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <Avatar className="h-9 w-9 border border-zinc-200 dark:border-zinc-800">
                            <AvatarFallback className="bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 text-xs font-medium">
                              {user.name.charAt(0)}
                            </AvatarFallback>
                          </Avatar>
                          <div>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1">
                          ...
                          <span
                            className={`text-xs ${user.team === "Unassigned" ? "text-zinc-400" : "text-rose-500 font-medium"}`}
                          >
                        ...
                        <Badge
                          ...
                          className={
                            user.status === "Active"
                              ...
                              : "text-zinc-500 dark:text-zinc-400 border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50"
                          }
                        >
                          {user.status}
                        </Badge>
                      ...
                      <td className="px-6 py-4 text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent
                            align="end"
                            className="w-44 bg-white dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800 shadow-xl rounded-xl"
                          >
                            <DropdownMenuItem
                              onClick={() => {
                            ...
                            <DropdownMenuItem
                              onClick={async () =>
                                managedSocietyId &&
                                (await removeUser(user.id, managedSocietyId))
                              }
      ...
      <Dialog
        open={!!editingUserRole}
        onOpenChange={(open) => !open && setEditingUserRole(null)}
      >
        <DialogContent className="sm:max-w-md border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950">
          <DialogHeader>
            <DialogTitle className="text-zinc-900 dark:text-white">
          ...
          <div className="flex flex-col space-y-4 mt-4">
            <div>
              ...
              <select
                ...
                className="w-full bg-background dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-800 rounded-lg p-2.5 text-sm text-zinc-900 dark:text-zinc-300 outline-none focus:border-rose-500/50 focus:ring-1 focus:ring-rose-500 transition appearance-none"
              >
